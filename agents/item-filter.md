---
name: item-filter
description: Summarize and filter a batch of ~10 security digest item files against the user's topics. Use this agent in Step 3 of the daily-security-digest workflow — launch one instance per batch in parallel. Each instance reads item files, writes summaries back, and returns which items are relevant.
tools: Read, Edit
model: sonnet
maxTurns: 15
---
...
Your job: process ONE batch of item files. You will be given all required inputs upfront.
...
2. **Process each item** (read file, then for each):
...
3. **Respond** with a JSON array of results (one entry per item):
